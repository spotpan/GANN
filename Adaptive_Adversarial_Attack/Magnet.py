...
# === Define Magnet Encoder Layer ===
class MagnetEncoder(Module):
    def __init__(self, in_features, out_features, with_bias=True):
        ...
        self.lin_skip = nn.Linear(in_features, out_features, bias=with_bias)  # Skip connection layer

        self.reset_parameters()

    def reset_parameters(self):
        # Initialize parameters for linear layers
        stdv = 1. / math.sqrt(self.lin_src_to_dst.weight.size(1))
        self.lin_src_to_dst.weight.data.uniform_(-stdv, stdv)
        self.lin_dst_to_src.weight.data.uniform_(-stdv, stdv)
        self.lin_skip.weight.data.uniform_(-stdv, stdv)
        if self.lin_src_to_dst.bias is not None:
            self.lin_src_to_dst.bias.data.uniform_(-stdv, stdv)
        if self.lin_dst_to_src.bias is not None:
            self.lin_dst_to_src.bias.data.uniform_(-stdv, stdv)
        if self.lin_skip.bias is not None:
            self.lin_skip.bias.data.uniform_(-stdv, stdv)

    def forward(self, x, adj, adj_t):
        # Magnet encoder uses source-to-destination, destination-to-source, and skip connection
        src_to_dst = self.lin_src_to_dst(adj @ x)
        dst_to_src = self.lin_dst_to_src(adj_t @ x)
        skip_connection = self.lin_skip(x)

        return src_to_dst + dst_to_src + skip_connection


# === Define Magnet Model using MagnetEncoder ===
class Magnet(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout=0.5, with_relu=True):
        super(Magnet, self).__init__()
        self.layer1 = MagnetEncoder(nfeat, nhid)
        self.layer2 = MagnetEncoder(nhid, nclass)
        self.dropout = dropout
        self.with_relu = with_relu

    def forward(self, x, adj):
        adj_t = adj.T  # Transpose for destination-to-source connections

        # First layer with activation
        x = self.layer1(x, adj, adj_t)
        if self.with_relu:
            x = F.relu(x)
        ...
        x = self.layer2(x, adj, adj_t)

        return F.log_softmax(x, dim=1)

    def initialize(self):
        # Reinitialize weights for both encoder layers
...
class BaseMeta(Module):
    ...
    def __init__(self, nfeat, hidden_sizes, nclass, nnodes, dropout, train_iters, attack_features, lambda_, device, with_bias=False, lr=0.01, with_relu=False):
        ...
        self.magnet = Magnet(nfeat=nfeat,
                       nhid=hidden_sizes[0],
                       nclass=nclass,
                       dropout=0.5,
                       with_relu=False)
        ...
        self.surrogate_optimizer = optim.Adam(self.magnet.parameters(), lr=lr, weight_decay=5e-4)
...
class Metattack(BaseMeta):
    ...
    def get_meta_grad(self, features, adj_norm, idx_train, idx_unlabeled, labels, labels_self_training):
        ...
        print(f'Magnet loss on unlabled data: {loss_test_val.item()}')
        print(f'Magnet acc on unlabled data: {utils.accuracy(output[idx_unlabeled], labels[idx_unlabeled]).item()}')
...
def test(adj):
    ...
    magnet = Magnet(nfeat=features.shape[1],
              nhid=args.hidden,
    ...
    optimizer = optim.Adam(magnet.parameters(),
...
def main():
    ...
    print('=== testing Magnet on attacked graph ===')
    for i in range(runs):
