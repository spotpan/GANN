...
n_features = torch.FloatTensor(n_features.toarray()).to(device)
...
print(100*np.mean(all_val_acc), 100*np.std(all_val_acc), 100*np.mean(all_test_acc), 100*np.std(all_test_acc))
