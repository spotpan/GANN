...
def get_3hop_neighbors(graph, nodes):
    """Get 3-hop neighbors in each possible role."""
    roles = {
        'succ_succ_succ': list(get_neighbors(graph, get_neighbors(graph, nodes, 'successor'), 'successor', depth=1)),
        'succ_succ_pred': list(get_neighbors(graph, get_neighbors(graph, nodes, 'successor'), 'predecessor', depth=1)),
        'succ_pred_pred': list(get_neighbors(graph, get_neighbors(graph, nodes, 'predecessor'), 'predecessor', depth=1)),
        'succ_pred_succ': list(get_neighbors(graph, get_neighbors(graph, nodes, 'predecessor'), 'successor', depth=1)),
        'pred_pred_pred': list(get_neighbors(graph, get_neighbors(graph, nodes, 'predecessor'), 'predecessor', depth=1)),
        'pred_pred_succ': list(get_neighbors(graph, get_neighbors(graph, nodes, 'predecessor'), 'successor', depth=1)),
        'pred_succ_succ': list(get_neighbors(graph, get_neighbors(graph, nodes, 'successor'), 'successor', depth=1)),
        'pred_succ_pred': list(get_neighbors(graph, get_neighbors(graph, nodes, 'successor'), 'predecessor', depth=1)),
    }

    # Check if any roles are missing nodes, and provide a warning if empty
    for role, nodes in roles.items():
        if not nodes:
            print(f"Warning: No nodes found for role '{role}'")
...
def visualize_3hop_strong_ties(clean_adj, attacked_adj, idx_train):
    """Visualize graph with 3-hop neighborhood distribution."""
    ...
    # Define 3-hop roles
    roles = {
        'succ_succ_succ': list(get_neighbors(G_clean, idx_train, 'successor', depth=3)),
        'succ_succ_pred': list(get_neighbors(G_clean, get_neighbors(G_clean, idx_train, 'successor', depth=2), 'predecessor')),
        'succ_pred_pred': list(get_neighbors(G_clean, get_neighbors(G_clean, idx_train, 'successor', depth=1), 'predecessor', depth=2)),
        'succ_pred_succ': list(get_neighbors(G_clean, get_neighbors(G_clean, idx_train, 'successor', depth=1), 'successor', depth=1)),
        'pred_pred_pred': list(get_neighbors(G_clean, idx_train, 'predecessor', depth=3)),
        'pred_pred_succ': list(get_neighbors(G_clean, get_neighbors(G_clean, idx_train, 'predecessor', depth=2), 'successor')),
        'pred_succ_succ': list(get_neighbors(G_clean, get_neighbors(G_clean, idx_train, 'predecessor', depth=1), 'successor', depth=2)),
        'pred_succ_pred': list(get_neighbors(G_clean, get_neighbors(G_clean, idx_train, 'predecessor', depth=1), 'predecessor', depth=1)),
    }

    # Calculate distributions
    total_distribution, strong_tie_distribution = get_strong_tie_distribution(roles, strong_tie_nodes)

    # Prepare the plot
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8), gridspec_kw={'width_ratios': [3, 1]})

    # ======= Graph Visualization (Left Side) ========
    pos = generate_layout(G_clean, roles, idx_train)
    role_colors = role_colors_map()

    # Draw nodes with appropriate colors and shapes
    for role, (color, shape) in role_colors.items():
        nx.draw_networkx_nodes(G_clean, pos, nodelist=roles[role], 
                               node_color=color, node_shape=shape, node_size=120, ax=ax1)

    # Draw training nodes at the center
    nx.draw_networkx_nodes(G_clean, pos, nodelist=list(idx_train), 
                           node_color='gray', node_size=200, edgecolors='black', ax=ax1)

    # Overlay strong ties in red
    strong_tie_overlay = [node for node in strong_tie_nodes if node in pos]
    nx.draw_networkx_nodes(G_clean, pos, nodelist=strong_tie_overlay, 
                           node_color='red', node_size=150, edgecolors='black', ax=ax1)

    # Draw edges safely
    draw_edges_safely(G_clean, pos, added_edges, removed_edges, ax=ax1)

    # Add dataset name and perturbation percentage to the title
    perturbation_percentage = args.ptb_rate * 100  # Convert to percentage
    ax1.set_title(f"3-Hop Graph Visualization - {args.dataset}\nPerturbation: {perturbation_percentage:.2f}%", fontsize=16)


    # ======= Vertical Stacked Bar Chart (Right Side) ========
    roles_with_residual = list(roles.keys())
    total_counts = np.array([len(roles[role]) for role in roles_with_residual])
    ...
    # Plot thinner, narrower stacked bar chart along Y-axis
    ax2.barh(roles_with_residual, non_strong_tie_counts, height=0.3, color=bar_colors, alpha=0.5)
    ax2.barh(roles_with_residual, strong_tie_counts, height=0.3, left=non_strong_tie_counts, color='red')

    # Add labels directly inside the bars
    for i, role in enumerate(roles_with_residual):
        ax2.text(non_strong_tie_counts[i] / 2, i, f'{role}', 
                 va='center', ha='center', fontsize=10, color='black')

    # Formatting the bar chart
    ax2.set_xlabel('Number of Nodes', fontsize=12)
    ax2.set_yticks([])  # Remove Y-axis ticks to cancel the Y-axis labels
...
def generate_layout(graph, roles, idx_train):
    """Generate layout with separation for each 3-hop role."""
    pos = {}
    center = (0, 0)
    base_radius = 2.0
    increment = 4.0

    # Define radius values for each 3-hop role
    role_radii = {
        'succ_succ_succ': base_radius + increment * 1,
        'succ_succ_pred': base_radius + increment * 2,
        'succ_pred_pred': base_radius + increment * 3,
        'succ_pred_succ': base_radius + increment * 4,
        'pred_pred_pred': base_radius + increment * 5,
        'pred_pred_succ': base_radius + increment * 6,
        'pred_succ_succ': base_radius + increment * 7,
        'pred_succ_pred': base_radius + increment * 8,
    }

    # Generate positions in circular layout for each 3-hop role
    for role, nodes in roles.items():
        if role in role_radii:
            radius = role_radii[role]
            pos.update(generate_positions_in_circle(nodes, center, radius))

    # Place training nodes at the center
    pos.update(generate_positions_in_circle(idx_train, center, base_radius - increment))

    return pos

def generate_positions_in_circle(nodes, center, radius, random_perturbation=0.2):
    """Generate positions for nodes in a circle with slight random perturbation."""
    positions = {}
...
def role_colors_map():
    """Return colors and markers for roles for 3-hop neighbors."""
    return {
        'succ_succ_succ': ('purple', 'o'),
        'succ_succ_pred': ('orange', 'h'),
        'succ_pred_pred': ('lightgreen', 'v'),
        'succ_pred_succ': ('cyan', 'p'),
        'pred_pred_pred': ('pink', 'X'),
        'pred_pred_succ': ('steelblue', 'd'),
        'pred_succ_succ': ('gold', '^'),
        'pred_succ_pred': ('brown', 's'), 
    }
def main():
    modified_adj = model(features, adj, labels, idx_train, idx_unlabeled, perturbations, ll_constraint=False).detach()
    visualize_3hop_strong_ties(adj, modified_adj, idx_train)
