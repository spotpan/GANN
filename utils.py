...
def get_adj(dataset, require_lcc=True):
    ...
    if require_lcc:
        ...
        #print("lcc",len(lcc))
        #print("found weakly connected component") 
    ...
    if(np.abs(_A_obs - _A_obs.T).sum() == 0): print("input graph is symmetric")
    if(np.abs(_A_obs - _A_obs.T).sum() != 0): print("input graph is not symmetric")
...
def largest_connected_components(adj, n_components=1):
    ...
    Parameters
    """
    _, component_indices = sp.csgraph.connected_components(adj,directed=True,connection="weak")
